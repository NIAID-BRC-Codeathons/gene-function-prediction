...
#                   ];
#
# Packaged counts (as usaged by most functions in this package):
#
...
@aa_order    = @gjoseqlib::aa_n_codon_order;
...
sub version
{
    ...
    my $bin;
    my $version;
    if ( $_ && defined( $bin = `which '$_'` )
            && $bin =~ /\S/
            && ( chomp $bin, 1 )       # a true value, even without a newline
            && open( FH, "$bin -v |" )
       )
    {
...
#  $ncpu = n_cpu()
#-------------------------------------------------------------------------------
#sub n_cpu { local $_ = `sysctl -n hw.ncpu`; chomp; $_ }


sub n_cpu {
    local $_;
    # Try Mac OS X sysctl first
    $_ = `sysctl -n hw.ncpu 2>/dev/null`;
    chomp;
    return $_ if $_ =~ /^\d+$/;
    ...
    $_ = `grep -c ^processor /proc/cpuinfo 2>/dev/null`;
    chomp;
    return $_ if $_ =~ /^\d+$/;
    
    print STDERR "NPROC set to $_\n\n";
...
sub single_quote_wrap { local $_ = shift; s/'/'"'"'/g; "'$_'" }
